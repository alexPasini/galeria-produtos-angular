...
import {
  ...
  ProductTypeSelected,
  ProductTypeResponse,
} from '../shared/types/card-products.types';
...
export class MainPageComponent {
  ...
  constructor(private apiService: ApiService) {
    this.apiService.getProducts().then((products: ProductTypeResponse[]) => {

      this.productsExample = products.map((product) => {

        return {
          id: product.id,
          name: product.name,
          price: product.price,
          images: product.images.map((image) => 'http://127.0.0.1:8000'+image.image),
        };
      });
      // this.productsExample = products;
    });
  }
